...
**`AboutWindowController`** — Static factory (`createWindow()`) building a 360x400 About window displaying the app icon, name, **dynamic version and build number** (read from `Bundle.main` `CFBundleShortVersionString` and `CFBundleVersion` at runtime), a brief description, copyright/license line (© 2026 Malcolm Taylor · MIT License), a scrollable open-source acknowledgements section with full MIT license text for both WhisperKit (Argmax, Inc.) and OpenAI Whisper, and a clickable "View HushType on GitHub" hyperlink at the bottom (an attributed `.link` string on a selectable `NSTextField`, opens the repo in the browser).
...
The DMG includes a branded Finder layout (AppleScript-configured) with the app, an Applications alias for drag-to-install, the User Guide (PDF preferred, docx fallback), and the MIT `LICENSE` (copied in as `LICENSE.txt`). A custom background image (`dmg-background.png`, 1320×880 @2x Retina) is stored in a hidden `.background` directory inside the DMG and shows a dashed arrow between the app and Applications icons, with "Drag to Applications to install" text. The 660×440 window layout positions HushType.app at (140, 185), Applications at (520, 185), the User Guide at (250, 345), and LICENSE.txt at (410, 345). The `release.sh` script converts the docx to PDF using LibreOffice (`soffice --headless`) or pandoc before building the DMG.
